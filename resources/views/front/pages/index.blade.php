    ...
    <div class="content-container">
        ...
        <div class="categories">
            ...
            <div class="categories_btns">
                <a href="{{url('/')}}"><button id="AllBTN" class="selected_category categories_left">All</button></a>
                @foreach($categories as $category)
                    <a href="{{route('category.select',['category_name'=>$category->name])}}"><button style="margin-left: 50px;">{{$category->name}}</button></a>
                @endforeach
            </div>
        </div>
        <div id="posts_container">
            @foreach($posts as $post)
                {{ optional($post->tags->first())->name }}
...
{{--                        <img data-post-id="{{$post->id}}" class="post_h3" src="`{{asset('imga/program.jpg')}}`">--}}
